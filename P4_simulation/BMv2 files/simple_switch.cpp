...
SimpleSwitch::SimpleSwitch(bool enable_swap, port_t drop_port)
  : Switch(enable_swap),
    ...
    input_buffer(new InputBuffer(
        50000 /* normal capacity */, 50000 /* resubmit/recirc capacity */)),
#ifdef SSWITCH_PRIORITY_QUEUEING_ON
    egress_buffers(nb_egress_threads,
                   50000, EgressThreadMapper(nb_egress_threads),
...
bool dequeued_pkt = false; //elbediwy added this
  while (1) {
    std::unique_ptr<Packet> packet = NULL;
    size_t port;
...
// Elbediwy : if we have a valid pointer to a packet, go fetch this packet from the buffer and process it.
if(TM_buffer_obj.valid_pop(packet) == 1) // added line
{
//  BMLOG_DEBUG("Elbediwy test : Started the Egress part of the simple_switch")
    if (packet == nullptr) break;
